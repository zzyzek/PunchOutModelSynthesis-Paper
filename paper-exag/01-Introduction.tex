...
\begin{itemize}
  ...
  \item Can reliably find realizations on arbitrarily sized grids with tile constraints that have finite influence length
\end{itemize}

and primary drawbacks that:

\begin{itemize}
  \item Has limited success for tile constraints that have long range influence length
\end{itemize}
...
\begin{itemize}
  ...
  \item Explore the Tile Arc Consistent Correlation Length (TACCL) (Section 2), a heuristic for tile influence length,
...
If there is a finite length of influence that one cell's tile choice has over another,
...
The influence distance informs the block size choice as block sizes chosen large enough allow
...
Even under random configuration assumptions, the issue can be further complicated if the influence length is not
finite, or finite but large.
Even though the general problem is likely intractable, or the finite influence length assumption is
