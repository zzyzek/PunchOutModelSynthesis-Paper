...
\begin{table}[h]
  ...
  \begin{tabular}[t]{l|ccccc}
    ...
    \hline
      ...
      Soften Size & 8 & 12-24 & 8 & 8-24 & 1-8 \\
...
The tile size, super tile window size, super tile window overlap parameter and boundary conditions are provided as they are used in the automatic tile constraint generation
for the resulting tile counts listed.
...
Also provided are the Block Choice Scheduler (BCS), soften size and blocks sizes that were used to generate the example outputs in Figure \ref{fig:2dexamples} and Figure \ref{fig:brutal_plum}.
...
The soften size is specific to the Breakout Model Synthesis (BMS) block level solver and the values shown in Table \ref{table:tilesets} were chosen through experimentation.
We document the soften size for completeness but a more thorough investigation of the soften size's impact on solvability
is beyond the scope of this paper.
%the reader is referred to 
%and the reader
%is referred to Algorithm \ref{alg:bms} in Appendix A or to Hoetzlein's \texttt{just\_math} 
...
\begin{figure*}[ht]
  \includegraphics[width=\textwidth]{2d_taccl_exemplar_out_fig.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{bp_taccl_ts_example.pdf}
...
An example output for the 2BMMV for a 128x128 grid using a 48x48 block size and uniform block choice schedule can be seen in Figure \ref{fig:2dexamples} (label \textit{l}).
